...
} from "@src/expenses/calculateBalance";
...
describe("ExpenseService", () => {
  ...
  describe("create", () => {
    it("should create expense without splits (solo expense)", async () => {
      ...
      vi.mocked(HouseholdMemberRepo.isExistingUser).mockResolvedValue(
        {} as any,
      );
      ...
      expect(HouseholdMemberRepo.isExistingUser).toHaveBeenCalledWith(
        "user-123",
        "household-123",
      );
    ...
    it("should create expense with splits (shared expense)", async () => {
      ...
      vi.mocked(ExpenseRepo.create).mockResolvedValue(mockExpense);
      vi.mocked(ExpenseSplitService.bulkCreate).mockResolvedValue(
        mockSplits as any,
      );
      ...
      expect(ExpenseSplitService.bulkCreate).toHaveBeenCalledWith([
        { expenseId: "expense-456", userId: "user-1", shareAmount: 50.0 },
        { expenseId: "expense-456", userId: "user-2", shareAmount: 50.0 },
        { expenseId: "expense-456", userId: "user-3", shareAmount: 50.0 },
      ]);
      expect(result).toMatchObject({
        status: StatusCodes.CREATED,
    ...
    it("should calculate equal splits correctly", async () => {
      ...
      const expectedShareAmount = 100.0 / 3; // 33.333...

      expect(ExpenseSplitService.bulkCreate).toHaveBeenCalledWith([
        {
          expenseId: "expense-789",
          userId: "user-1",
          shareAmount: expectedShareAmount,
        },
        {
          expenseId: "expense-789",
          userId: "user-2",
    ...
    it("should handle expense creation error", async () => {
      ...
      vi.mocked(ExpenseRepo.create).mockRejectedValue(
  ...
  describe("getExpensesByHousehold", () => {
    it("should get all expenses for household", async () => {
      ...
      vi.mocked(ExpenseRepo.getExpensesByHouseholdId).mockResolvedValue(
        mockExpenses as any,
      );

      const result =
        await ExpenseService.getExpensesByHousehold("household-123");

      expect(ExpenseRepo.getExpensesByHouseholdId).toHaveBeenCalledWith(
        "household-123",
      );
      expect(result).toMatchObject({
  ...
  describe("delete", () => {
    it("should successfully delete expense", async () => {
      ...
      vi.mocked(ExpenseRepo.getExpenseByExpenseId).mockResolvedValue(
        mockExpense as any,
      );
      ...
      expect(ExpenseRepo.getExpenseByExpenseId).toHaveBeenCalledWith(
        "expense-123",
      );
  ...
  describe("getBalances", () => {
    it("should calculate balances for household", async () => {
      const mockExpensesWithSplits = [
        {
          ...
          householdId: "household-123",
          splits: [
            { userId: "user-1", shareAmount: 50.0 },
            { userId: "user-2", shareAmount: 50.0 },
          ],
        },
      ];

      const mockBalances = [
        { userId: "user-1", name: "Alice", balance: 50.0 },
        { userId: "user-2", name: "Bob", balance: -50.0 },
      ];
      ...
      vi.mocked(ExpenseRepo.getExpensesWithSplits).mockResolvedValue(
        mockExpensesWithSplits as any,
      );
      ...
      vi.mocked(HouseholdMemberRepo.getByHouseholdId).mockResolvedValue(
        mockHouseholdMembers as any,
      );

      const result = await ExpenseService.getBalances("household-123");

      expect(ExpenseRepo.getExpensesWithSplits).toHaveBeenCalledWith(
        "household-123",
      );
