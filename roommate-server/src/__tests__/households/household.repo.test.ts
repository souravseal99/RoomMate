...
describe("HouseholdRepository", () => {
  ...
  describe("create", () => {
    it("should create household with admin role", async () => {
      ...
      const result = await HouseholdRepository.create(
        "My Apartment",
        "user-123",
        Role.ADMIN,
      );
  ...
  describe("getHouseholdByInviteCode", () => {
    it("should return household when found or null when not found", async () => {
      ...
      const result =
        await HouseholdRepository.getHouseholdByInviteCode("FOUND123");
      ...
      const notFound =
        await HouseholdRepository.getHouseholdByInviteCode("NOTFOUND");
  ...
  describe("getHouseholdById", () => {
    it("should return household when found or null when not found", async () => {
      ...
      const result =
        await HouseholdRepository.getHouseholdById("household-123");
      ...
      const notFound =
        await HouseholdRepository.getHouseholdById("non-existent");
      expect(notFound).toBeNull();
    });
  });
  ...
  describe("findNamesLikeByUser", () => {
    it("should find households with similar names for ADMIN users", async () => {
      ...
      const result = await HouseholdRepository.findNamesLikeByUser(
        "user-123",
        "My House",
      );
      ...
      expect(prisma.household.findMany).toHaveBeenCalledWith({
        where: {
          ...
          OR: [{ name: "My House" }, { name: { startsWith: "My House (" } }],
        },
        select: { name: true },
      });
