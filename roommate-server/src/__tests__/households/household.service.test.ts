...
describe("HouseholdService", () => {
  ...
  describe("create", () => {
    it("should successfully create household with unique name", async () => {
      ...
      vi.mocked(HouseholdRepository.create).mockResolvedValue(
      ...
      expect(HouseholdRepository.findNamesLikeByUser).toHaveBeenCalledWith(
        "user-123",
        "My House",
      );
    ...
    it("should create household with numbered suffix when name exists", async () => {
      ...
      const existingNames = [{ name: "My House" }, { name: "My House (1)" }];
    ...
    it("should reject creation when user not found", async () => {
      ...
      const result = await HouseholdService.create(
        "user-nonexistent",
        "My House",
      );
  ...
  describe("getHouseholdsByUser", () => {
    it("should successfully get all households for user", async () => {
      ...
      vi.mocked(UserRepo.getUserById).mockResolvedValue(mockUser);
      vi.mocked(HouseholdRepository.getHouseholdsByUser).mockResolvedValue(
        mockHouseholds as any,
      );

      const result = await HouseholdService.getHouseholdsByUser("user-123");

      expect(UserRepo.getUserById).toHaveBeenCalledWith("user-123");
      expect(HouseholdRepository.getHouseholdsByUser).toHaveBeenCalledWith(
        "user-123",
      );
      expect(result).toMatchObject({
    ...
    it("should reject when user not found", async () => {
      ...
      const result =
        await HouseholdService.getHouseholdsByUser("user-nonexistent");
  ...
  describe("delete", () => {
    it("should successfully delete household", async () => {
      ...
      vi.mocked(HouseholdRepository.delete).mockResolvedValue(
        mockDeleteResponse as any,
      );

      const result = await HouseholdService.delete("household-123");

      expect(HouseholdRepository.getHouseholdById).toHaveBeenCalledWith(
        "household-123",
      );
  ...
  describe("join", () => {
    it("should successfully join household with invite code", async () => {
      ...
      vi.mocked(HouseholdRepository.getHouseholdByInviteCode).mockResolvedValue(
        mockHousehold,
      );
      vi.mocked(UserRepo.getUserById).mockResolvedValue(mockUser);
      ...
      expect(HouseholdRepository.getHouseholdByInviteCode).toHaveBeenCalledWith(
        "INVITE12",
      );
      ...
      expect(HouseholdMemberRepo.join).toHaveBeenCalledWith(
        "user-456",
        "household-123",
      );
      expect(result).toMatchObject({
