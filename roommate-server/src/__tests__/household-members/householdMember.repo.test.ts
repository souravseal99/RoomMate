...
describe("HouseholdMemberRepo", () => {
  ...
  describe("isExistingUser", () => {
    it("should return member when user exists in household", async () => {
      ...
      const result = await HouseholdMemberRepo.isExistingUser(
        "user-456",
        "household-456",
      );
  ...
  describe("join", () => {
    it("should join household as MEMBER role", async () => {
      ...
      const result = await HouseholdMemberRepo.join(
        "user-789",
        "household-789",
      );
  ...
  describe("getByHouseholdId", () => {
    it("should get all members with user details", async () => {
      ...
      const result =
        await HouseholdMemberRepo.getByHouseholdId("household-123");
    ...
    it("should return empty array when no members", async () => {
      ...
      const result =
        await HouseholdMemberRepo.getByHouseholdId("household-empty");
